 ...
 for (const user of users) {
  ...
  if (dmSuccessful) {
   if (usersDMedToday.length < maxDMsPerDay) {
    ...
    await delay(18 * 60 * 60 * 1000); // Stop for a day (24 hours)
...
function sortUsers(
 ...
 function removeUsersWithMatchingIds(userArray, idsToRemove) {
  // Extract the ids from the objects in idsToRemove array
  const idsToRemoveArray = idsToRemove.map((item) => item.id);

  // Use the filter function to keep only the users whose IDs are not in idsToRemoveArray
